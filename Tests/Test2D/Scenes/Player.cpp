...
namespace test
{
	...
	void Player::Update()
	{
		...
		if (!Scenes::Get()->IsPaused())
		{
			...
			targetVelocity.m_x += (sprintDown ? RUN_SPEED : crouchDown ? CROUCH_SPEED : WALK_SPEED) * m_inputForward.GetAmount();
	...
	void Player::Decode(const Metadata &metadata)
	{
	}

	void Player::Encode(Metadata &metadata) const
	{
	}
}
