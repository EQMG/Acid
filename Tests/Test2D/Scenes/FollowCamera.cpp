...
namespace test
{
	const float FollowCamera::NEAR_PLANE = 0.1f;
	const float FollowCamera::FAR_PLANE = 2048.0f;
	const float FollowCamera::FIELD_OF_VIEW = 65.0f;

	const float FollowCamera::ROTATE_AGILITY = 32.0f;
	const float FollowCamera::PITCH_AGILITY = 32.0f;

	const float FollowCamera::VIEW_HEIGHT = 1.8f;

	const float FollowCamera::INFLUENCE_OF_JOYSTICK_DY = 4.5f;
	const float FollowCamera::INFLUENCE_OF_JOYSTICK_DX = 4.5f;

	const float FollowCamera::INFLUENCE_OF_MOUSE_DY = 10000.0f;
	const float FollowCamera::INFLUENCE_OF_MOUSE_DX = 10000.0f;

	const float FollowCamera::MAX_HORIZONTAL_CHANGE = 30.0f;
	const float FollowCamera::MAX_VERTICAL_CHANGE = 30.0f;

	const float FollowCamera::MAX_ANGLE_OF_ELEVATION = 85.0f;
	const float FollowCamera::MIN_ANGLE_OF_ELEVATION = -85.0f;
	...
	void FollowCamera::Update()
	{
		...
		auto scenePlayer = Scenes::Get()->GetStructure()->GetComponent<Player>();
	...
	void FollowCamera::CalculateHorizontalAngle()
	{
	...
	void FollowCamera::CalculateVerticalAngle()
	{
		float angleChange = 0.0f;
	...
	void FollowCamera::UpdateHorizontalAngle(const float &delta)
	{
	...
	void FollowCamera::UpdatePitchAngle(const float &delta)
	{
	...
	void FollowCamera::ReflectView(const float &height)
	{
