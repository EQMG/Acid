...
Inspector::Inspector(UiObject *parent) :
	UiPanel(parent, UiBound(Vector2f(1.0f, 0.0f), UiReference::TopRight, UiAspect::Position | UiAspect::Dimensions, Vector2f(0.3f, 1.0f)), ScrollBar::None),
...
{
	m_slider1.OnValue() += [this](float value)
	{
		Audio::Get()->SetGain(Audio::Type::Effect, value);
	};
	m_text1.OnValue() += [this](std::string text)
	{
