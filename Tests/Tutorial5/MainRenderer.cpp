...
public:
	...
	static Shader::VertexInput GetVertexInput(uint32_t baseBinding = 0) {
		std::vector<VkVertexInputBindingDescription> bindingDescriptions = {
			{baseBinding, sizeof(TutorialVertex5), VK_VERTEX_INPUT_RATE_VERTEX}
		};
		std::vector<VkVertexInputAttributeDescription> attributeDescriptions = {
			{0, baseBinding, VK_FORMAT_R32G32_SFLOAT, offsetof(TutorialVertex5, position)},
			{1, baseBinding, VK_FORMAT_R32G32B32_SFLOAT, offsetof(TutorialVertex5, color)},
			{0, baseBinding, VK_FORMAT_R32G32_SFLOAT, offsetof(TutorialVertex5, uv)}
		};
 ...
 */
class TutorialSubrender5 : public Subrender {
public:
	/*
	 * To follow the Vulkan tutorial, we have flipped the Y in the projection, and here
	 * we are also reversing the triangle
	 * cull mode as per https://vulkan-tutorial.com/Uniforbuffers/Descriptor_layout_and_buffer
	 */
	explicit TutorialSubrender5(const Pipeline::Stage &stage) :
		Subrender(stage),
		pipeline(stage, {"Tutorial/Shaders/tri5.vert", "Tutorial/Shaders/tri5.frag"},
			{TutorialVertex5::GetVertexInput()}, {}, PipelineGraphics::Mode::Polygon, PipelineGraphics::Depth::None,
...
void MainRenderer::Start() {
	...
	const std::vector<TutorialVertex5> vertices = {
	...
	auto square = AddSubrender<TutorialSubrender5>({0, 0});
