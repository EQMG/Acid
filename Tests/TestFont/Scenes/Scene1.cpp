...
namespace test
{
	Scene1::Scene1() :
		...
		m_texts(std::vector<std::pair<Vector2, std::unique_ptr<Text>>>()),
		...
		m_textUps(std::make_unique<Text>(Uis::Get()->GetContainer(), UiBound(Vector2(0.002f, 0.022f), UiBound::BOTTOM_LEFT, true), 1.1f, "UPS: 0", FontType::Resource("Fonts/ProximaNova", "Regular"), TEXT_JUSTIFY_LEFT))
	{
		...
		m_texts.emplace_back(Vector2(0.5f, 0.5f), std::make_unique<Text>(Uis::Get()->GetContainer(), UiBound(Vector2(0.5f, 0.5f), UiBound::CENTRE, false), 2.0f, content1,
	...
	void Scene1::Update()
	{
		...
		m_zoom *= powf(1.3f, 0.05f * Mouse::Get()->GetDeltaWheel());
		...
		{
			text.second->GetRectangle().SetPosition(m_zoom * ((text.first * Vector2(Display::Get()->GetAspectRatio(), 1.0f)) + m_offset));
			dynamic_cast<DriverConstant *>(text.second->GetScaleDriver())->SetConstant(m_zoom);
		}
