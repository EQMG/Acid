...
namespace test
{
	...
	void Pannable::UpdateObject()
	{
		...
		{
			m_offset.m_x -= Mouse::Get()->GetDeltaX() * Renderer::Get()->GetWindow()->GetAspectRatio() / m_zoom / Engine::Get()->GetDelta().AsSeconds();
			m_offset.m_y += Mouse::Get()->GetDeltaY() / m_zoom / Engine::Get()->GetDelta().AsSeconds();
		}

		for (auto &text : m_texts)
		{
			text.second->GetRectangle().SetPosition((m_zoom * (Vector2(text.first) + m_offset)) + Vector2(0.5f * Renderer::Get()->GetWindow()->GetAspectRatio(), 0.5f));
