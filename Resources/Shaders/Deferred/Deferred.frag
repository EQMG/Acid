...
//	vec4 textureOcclusion = texture(samplerOcclusion, fragmentUv);
	float textureShadow = texture(samplerShadows, fragmentUv).r;
	...
	if (!ignoreLighting && textureNormal.rgb != vec3(0.0f))
	{
		...
		for (int i = 0; i < scene.lightsCount; i++)
		{
            ...
            float ao = 1.0f; // textureOcclusion.r
			irradiance += radiance * ao * L0(N, L, V, roughness, metallic, textureColour.rgb);
		}
