...
namespace Demo
{
	...
	ManagerUis::ManagerUis() :
		...
		m_overlayStartup(nullptr),// new OverlayStartup(Uis::Get()->GetContainer())),
		m_overlayDebug(nullptr),//new OverlayDebug(Uis::Get()->GetContainer())),
		m_overlayPaused(nullptr)//new OverlayPaused(Uis::Get()->GetContainer()))
	{
	...
	{
		return false; //  m_overlayStartup->IsStarting() || m_overlayPaused->GetAlpha() != 0.0f;
	}
