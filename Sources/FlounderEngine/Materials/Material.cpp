...
namespace Flounder
{
		...
		m_textureMaterial(material)
	{
		...
		LinkString(1, LINK_GET_RES(GetTextureDiffuse()), LINK_SET(std::string, SetTextureDiffuse(Texture::Resource(v))));
		...
		LinkString(7, LINK_GET_RES(GetTextureMaterial()), LINK_SET(std::string, SetTextureMaterial(Texture::Resource(v))));
	}
