...
PipelineBlur::PipelineBlur(const Pipeline::Stage& pipelineStage, const float& blur, const FilterBlur::Type& blurType, const bool& toScreen, const float& inputScale, const float& outputScale)
	: PostPipeline(pipelineStage), m_filterBlurVertical(pipelineStage, Vector2(0.0f, blur), blurType), m_filterBlurHorizontal(pipelineStage, Vector2(blur, 0.0f), blurType), m_toScreen(toScreen), m_inputScale(inputScale), m_outputScale(outputScale), m_blur(blur),
	  m_output(nullptr), m_lastWidth(0), m_lastHeight(0)
{
}
...
void PipelineBlur::Render(const CommandBuffer& commandBuffer)
{
	...
	// faster
	// blur.
	//	Texture::CopyImage(dynamic_cast<Texture
	//*>(Renderer::Get()->GetAttachment("resolved"))->GetImage(),
	// m_halfRes->GetImage(),
	// m_halfRes->GetDeviceMemory(),
	// width
	/// 2,
	// height
	/// 2,
