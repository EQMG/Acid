...
RendererDeferred::RendererDeferred(const Pipeline::Stage& pipelineStage)
	: RenderPipeline(pipelineStage), m_pipeline(pipelineStage, {"Shaders/Deferred/Deferred.vert", "Shaders/Deferred/Deferred.frag"}, {}, GetDefines(), PipelineGraphics::Mode::Polygon, PipelineGraphics::Depth::None), m_brdf(ComputeBRDF(512)), m_skybox(nullptr),
	  m_irradiance(nullptr), m_prefiltered(nullptr), m_fog(Colour::White, 0.001f, 2.0f, -0.1f, 0.3f)
{
...
std::unique_ptr<Texture> RendererDeferred::ComputeBRDF(const uint32_t& size)
{
	...
	pixels
	=
	result->GetPixels(width,
	height,
	1);
	Texture::WritePixels(filename,
	pixels.get(),
	width,
	height);*/
#endif
...
std::unique_ptr<Cubemap> RendererDeferred::ComputePrefiltered(const std::shared_ptr<Cubemap>& source, const uint32_t& size)
{
	...
	// mip
	// increases.
	auto result = std::make_unique<Cubemap>(size, size, nullptr, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_FILTER_LINEAR, VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, VK_SAMPLE_COUNT_1_BIT,
											true, true);
	...
	// pushHandler
	//=
	// PushHandler(*compute.GetShaderProgram()->GetUniformBlock("PushObject"));

	// Updates
	// descriptors.
	DescriptorsHandler descriptorSet = DescriptorsHandler(compute);
	//	descriptorSet.Push("PushObject",
	// pushHandler);
	descriptorSet.Push("outColour", result.get());
	descriptorSet.Push("samplerColour", source);
	descriptorSet.Update(compute);
	...
	// pipeline.
	descriptorSet.BindDescriptor(commandBuffer, compute);
	//	pushHandler.BindPush(commandBuffer,
	// compute);
	compute.CmdRender(commandBuffer, result->GetWidth(), result->GetHeight());
	commandBuffer.End();
	commandBuffer.SubmitIdle();
