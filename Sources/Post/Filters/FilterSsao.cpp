...
FilterSsao::FilterSsao(const Pipeline::Stage& pipelineStage) : PostFilter(pipelineStage, {"Shaders/Post/Default.vert", "Shaders/Post/Ssao.frag"}, GetDefines()), m_noise(ComputeNoise(SSAO_NOISE_DIM)), m_kernel(SSAO_KERNEL_SIZE)
{
...
std::shared_ptr<Texture> FilterSsao::ComputeNoise(const uint32_t& size)
{
	...
	for(uint32_t i = 0; i < size * size; i++)
		{
			Vector3 noise = Vector3(Maths::Random(-1.0f, 1.0f), Maths::Random(-1.0f, 1.0f),
									0.0f); // Vector3(float(i) / float(size * size), 0.0f, 0.0f);
			noise = noise.Normalize();
			ssaoNoise[i] = Colour(noise, 1.0f);
		}

	auto result =
		std::make_shared<Texture>(size, size, std::unique_ptr<uint8_t[]>(reinterpret_cast<uint8_t*>(ssaoNoise.data())), VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_FILTER_NEAREST);
