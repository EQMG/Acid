...
{
Renderer::Renderer()
	: m_renderManager(nullptr), m_swapchain(nullptr), m_pipelineCache(VK_NULL_HANDLE), m_commandPool(VK_NULL_HANDLE), m_currentFrame(0), m_instance(std::make_unique<Instance>()), m_physicalDevice(std::make_unique<PhysicalDevice>(m_instance.get())),
	  m_surface(std::make_unique<Surface>(m_instance.get(), m_physicalDevice.get())), m_logicalDevice(std::make_unique<LogicalDevice>(m_instance.get(), m_physicalDevice.get(), m_surface.get()))
{
