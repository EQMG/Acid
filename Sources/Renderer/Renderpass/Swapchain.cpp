...
Swapchain::Swapchain(const VkExtent2D& extent)
	: m_extent(extent), m_presentMode(VK_PRESENT_MODE_FIFO_KHR), m_imageCount(0), m_preTransform(VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR), m_compositeAlpha(VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR), m_swapchain(VK_NULL_HANDLE), m_fenceImage(VK_NULL_HANDLE),
...
VkResult Swapchain::AcquireNextImage(const VkSemaphore& presentCompleteSemaphore)
{
	...
	//	Renderer::CheckVk(vkWaitForFences(logicalDevice->GetLogicalDevice(),
	// 1,
	//&m_fenceImage,
	// VK_TRUE,
	// std::numeric_limits<uint64_t>::max()));

	//	Renderer::CheckVk(vkResetFences(logicalDevice->GetLogicalDevice(),
	// 1,
	//&m_fenceImage));
	return VK_SUCCESS;
}
