...
namespace acid
{
DescriptorsHandler::DescriptorsHandler() : m_shader(nullptr), m_pushDescriptors(false), m_descriptorSet(nullptr), m_changed(false) {}

DescriptorsHandler::DescriptorsHandler(const Pipeline& pipeline) : m_shader(pipeline.GetShaderProgram()), m_pushDescriptors(pipeline.IsPushDescriptors()), m_descriptorSet(std::make_unique<DescriptorSet>(pipeline)), m_changed(true) {}
			...
			for(const auto& [descriptorName, descriptor] : m_descriptors)
				{
					...
					auto writeDescriptor = descriptor.descriptor->GetWriteDescriptor(descriptor.location, *descriptorType, m_pushDescriptors ? VK_NULL_HANDLE : m_descriptorSet->GetDescriptorSet(), descriptor.offsetSize);
	...
	if(m_pushDescriptors)
		{
			...
			Instance::FvkCmdPushDescriptorSetKHR(logicalDevice->GetLogicalDevice(), commandBuffer.GetCommandBuffer(), pipeline.GetPipelineBindPoint(), pipeline.GetPipelineLayout(), 0, static_cast<uint32_t>(m_writeDescriptorSets.size()), m_writeDescriptorSets.data());
		}
	else
		{
