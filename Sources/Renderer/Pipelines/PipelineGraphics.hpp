...
class ACID_EXPORT PipelineGraphics : public Pipeline
{
  public:
	...
	/// pushed.
	/// </param>
	PipelineGraphics(Stage stage, std::vector<std::string> shaderStages, std::vector<Shader::VertexInput> vertexInputs, std::vector<Shader::Define> defines = {}, const Mode& mode = Mode::Polygon, const Depth& depthMode = Depth::ReadWrite,
					 const VkPrimitiveTopology& topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, const VkPolygonMode& polygonMode = VK_POLYGON_MODE_FILL, const VkCullModeFlags& cullMode = VK_CULL_MODE_BACK_BIT, const VkFrontFace& frontFace = VK_FRONT_FACE_CLOCKWISE,
	...
	const VkPrimitiveTopology& GetTopology() const
	{
		return m_topology;
	}

	const VkPolygonMode& GetPolygonMode() const
	{
...
{
  public:
	explicit PipelineGraphicsCreate(std::vector<std::string> shaderStages = {}, std::vector<Shader::VertexInput> vertexInputs = {}, std::vector<Shader::Define> defines = {}, const PipelineGraphics::Mode& mode = PipelineGraphics::Mode::Polygon,
		...
		: m_shaderStages(std::move(shaderStages)), m_vertexInputs(std::move(vertexInputs)), m_defines(std::move(defines)), m_mode(mode), m_depth(depth), m_topology(topology), m_polygonMode(polygonMode), m_cullMode(cullMode), m_frontFace(frontFace),
	...
	const PipelineGraphics::Mode& GetMode() const
	{
		return m_mode;
	}

	const PipelineGraphics::Depth& GetDepth() const
	{
		return m_depth;
	}
	...
	const bool& GetPushDescriptors() const
	{
		return m_pushDescriptors;
	}
