...
PipelineCompute::PipelineCompute(std::string shaderStage, std::vector<Shader::Define> defines, const bool& pushDescriptors)
	: m_shaderStage(std::move(shaderStage)), m_defines(std::move(defines)), m_pushDescriptors(pushDescriptors), m_shader(std::make_unique<Shader>(m_shaderStage)), m_shaderModule(VK_NULL_HANDLE), m_shaderStageCreateInfo({}), m_descriptorSetLayout(VK_NULL_HANDLE),
	  m_descriptorPool(VK_NULL_HANDLE), m_pipeline(VK_NULL_HANDLE), m_pipelineLayout(VK_NULL_HANDLE), m_pipelineBindPoint(VK_PIPELINE_BIND_POINT_COMPUTE)
{
