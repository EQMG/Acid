...
namespace acid
{
Shader::Shader(std::string name) : m_name(std::move(name)), m_lastDescriptorBinding(0) {}
...
void Shader::ProcessShader()
{
			...
			switch(uniform.m_glType)
				{
					...
					case 0x9055: // GL_IMAGE_2D_MULTISAMPLE
						...
						m_descriptorSetLayouts.emplace_back(Texture::GetDescriptorSetLayout(static_cast<uint32_t>(uniform.m_binding), descriptorType, uniform.m_stageFlags, 1));
						break;
					case 0x8B60: // GL_SAMPLER_CUBE
					case 0x9050: // GL_IMAGE_CUBE
						descriptorType = uniform.m_writeOnly ? VK_DESCRIPTOR_TYPE_STORAGE_IMAGE : VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
						m_descriptorSetLayouts.emplace_back(Cubemap::GetDescriptorSetLayout(static_cast<uint32_t>(uniform.m_binding), descriptorType, uniform.m_stageFlags, 1));
						break;
					default:
						break;
				}
	...
	// by
	// binding.
	std::sort(m_descriptorSetLayouts.begin(), m_descriptorSetLayouts.end(), [](const VkDescriptorSetLayoutBinding& l, const VkDescriptorSetLayoutBinding& r) { return l.binding < r.binding; });
...
void Shader::LoadUniform(const glslang::TProgram& program, const VkShaderStageFlags& stageFlag, const int32_t& i)
{
	...
	m_uniforms.emplace(program.getUniformName(i), Uniform(program.getUniformBinding(i), program.getUniformBufferOffset(i), -1, program.getUniformType(i), qualifier.readonly, qualifier.writeonly, stageFlag));
}
...
{
	// glslang::TType::computeNumComponents
	// is
	// available
	// but
	// has
