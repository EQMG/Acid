...
PhysicalDevice::PhysicalDevice(const Instance* instance) : m_instance(instance), m_physicalDevice(VK_NULL_HANDLE), m_properties({}), m_features({}), m_memoryProperties({}), m_msaaSamples(VK_SAMPLE_COUNT_1_BIT)
{
...
void PhysicalDevice::LogVulkanDevice(const VkPhysicalDeviceProperties& physicalDeviceProperties, const std::vector<VkExtensionProperties>& extensionProperties)
{
	...
	uint32_t supportedVersion[] = {VK_VERSION_MAJOR(physicalDeviceProperties.apiVersion), VK_VERSION_MINOR(physicalDeviceProperties.apiVersion), VK_VERSION_PATCH(physicalDeviceProperties.apiVersion)};
