...
void CallbackMonitor(GLFWmonitor* monitor, int32_t event)
{
	if(event == GLFW_CONNECTED)
	...
	else if(event == GLFW_DISCONNECTED)
		{
			...
			monitors.erase(std::remove_if(monitors.begin(), monitors.end(), [&](Monitor& m) { return monitor == m.GetMonitor(); }), monitors.end());
...
Window::Window()
	: m_windowWidth(1080), m_windowHeight(720), m_fullscreenWidth(0), m_fullscreenHeight(0), m_aspectRatio(1.5f), m_positionX(0), m_positionY(0), m_title("Acid Loading..."), m_borderless(false), m_resizable(true), m_floating(false), m_fullscreen(false), m_closed(false),
	  m_focused(true), m_iconified(false), m_window(nullptr)
{
	// Set
	// the
	// error
	...
	glfwWindowHint(GLFW_VISIBLE,
				   GLFW_FALSE); // The window will stay hidden until after creation.

	glfwWindowHint(GLFW_CLIENT_API,
				   GLFW_NO_API); // Disable context creation.
	...
	glfwWindowHint(GLFW_STENCIL_BITS,
				   8); // Fixes 16 bit stencil bits in macOS.
	glfwWindowHint(GLFW_STEREO,
				   GLFW_FALSE); // No stereo view!
