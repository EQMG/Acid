...
class ACID_EXPORT Window : public Module
{
  public:
	...
	static Window* Get()
	{
		return Engine::Get()->GetModuleManager().Get<Window>();
	}
	...
	/// position.
	/// </returns>
	const uint32_t& GetPositionX() const
	{
		return m_positionX;
	}
	...
	/// position.
	/// </returns>
	const uint32_t& GetPositionY() const
	{
		return m_positionY;
	}
	...
	Delegate<void(uint32_t, bool)>& GetOnMonitorConnect()
	{
		return m_onMonitorConnect;
	}

	Delegate<void()>& GetOnClose()
	{
		return m_onClose;
	}

	Delegate<void(bool)>& GetOnIconify()
	{
		return m_onIconify;
	}
