...
class ACID_EXPORT Joysticks : public Module
{
  public:
	...
	{
		return Engine::Get()->GetModuleManager().Get<Joysticks>();
	}
	...
	Delegate<void(uint32_t, bool)>& GetOnConnect()
	{
		return m_onConnect;
	}

	Delegate<void(uint32_t, uint32_t, InputAction)>& GetOnButton()
	{
		return m_onButton;
	}

	Delegate<void(uint32_t, uint32_t, float)>& GetOnAxis()
	{
		return m_onAxis;
	}

	Delegate<void(uint32_t, uint32_t, BitMask<JoystickHat>)>& GetOnHat()
	{
		return m_onHat;
	}
