...
class ACID_EXPORT Mouse : public Module
{
  public:
	...
	static Mouse* Get()
	{
		return Engine::Get()->GetModuleManager().Get<Mouse>();
	}
	...
	Vector2 GetPosition()
	{
		return Vector2(GetPositionX(), GetPositionY());
	}
	...
	{
		return Vector2(GetDeltaX(), GetDeltaY());
	}
	...
	/// delta.
	/// </returns>
	const float& GetDeltaWheel() const
	{
	...
	/// hidden.
	/// </returns>
	const bool& IsCursorHidden() const
	{
	...
	Delegate<void(MouseButton, InputAction, BitMask<InputMod>)>& GetOnButton()
	{
		return m_onButton;
	}

	Delegate<void(float, float)>& GetOnPosition()
	{
		return m_onPosition;
	}

	Delegate<void(bool)>& GetOnEnter()
	{
		return m_onEnter;
	}

	Delegate<void(float, float)>& GetOnScroll()
	{
		return m_onScroll;
	}

	Delegate<void(std::vector<std::string>)>& GetOnDrop()
	{
		return m_onDrop;
	}
