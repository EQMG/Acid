...
class ACID_EXPORT Text : public UiObject
{
  public:
	...
	/// text.
	/// </param>
	Text(UiObject* parent, const UiBound& rectangle, const float& fontSize, std::string text, std::shared_ptr<FontType> fontType = FontType::Create("Fonts/ProximaNova", "Regular"), const Justify& justify = Justify::Left, const float& maxWidth = 1.0f,
	...
	/// lines.
	/// </returns>
	const uint32_t& GetNumberLines() const
	{
	...
	/// justify.
	/// </returns>
	const Justify& GetJustify() const
	{
		return m_justify;
	}
	...
	/// text.
	/// </returns>
	const std::shared_ptr<FontType>& GetFontType() const
	{
		return m_fontType;
	}
	...
	/// text.
	/// </param>
	void SetBorderColour(const Colour& borderColour)
	{
		m_borderColour = borderColour;
	}

	IDriver<float>* GetGlowDriver() const
	{
	...
	{
		SetGlowDriver(new T(std::forward<Args>(args)...));
	}

	IDriver<float>* GetBorderDriver() const
	{
		return m_borderDriver.get();
	}
	...
	{
		SetBorderDriver(new T(std::forward<Args>(args)...));
	}
  ...
  private:
	...
	class Line
	{
	  public:
		...
		Line(const float& spaceWidth, const float& maxLength) : m_maxLength(maxLength), m_spaceSize(spaceWidth), m_currentWordsLength(0.0f), m_currentLineLength(0.0f) {}
