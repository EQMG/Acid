...
FontType::FontType(std::string filename, std::string style, const bool& load) : m_filename(std::move(filename)), m_style(std::move(style)), m_texture(nullptr), m_metadata(nullptr), m_storageGlyphs(nullptr), m_instanceBuffer(nullptr), m_glyphInstances(nullptr), m_instances(0)
{
	if(load)
		{
			FontType::Load();
