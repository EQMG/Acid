...
namespace acid
{
Text::Text(UiObject* parent, const UiBound& rectangle, const float& fontSize, std::string text, std::shared_ptr<FontType> fontType, const Justify& justify, const float& maxWidth, const Colour& textColour, const float& kerning, const float& leading)
	: UiObject(parent, rectangle), m_model(nullptr), m_numberLines(0), m_string(std::move(text)), m_newString({}), m_justify(justify), m_fontType(std::move(fontType)), m_maxWidth(maxWidth), m_kerning(kerning), m_leading(leading), m_textColour(textColour),
	  m_borderColour(Colour::White), m_solidBorder(false), m_glowBorder(false), m_glowDriver(std::make_unique<DriverConstant<float>>(0.0f)), m_glowSize(0.0f), m_borderDriver(std::make_unique<DriverConstant<float>>(0.0f)), m_borderSize(0.0f)
{
...
void Text::NormalizeQuad(Vector2& bounding, std::vector<VertexModel>& vertices) const
{
	...
	//	maxY
	//=
	// static_cast<float>(GetFontType()->GetMetadata()->GetMaxSizeY())
	//*
	// m_numberLines;
