...
namespace acid
{
	...
	private:
		std::shared_ptr<JointData> LoadJointData(const std::shared_ptr<Node> &jointNode, const bool &isRoot);

		std::shared_ptr<JointData> ExtractMainJointData(const std::shared_ptr<Node> &jointNode, const bool &isRoot);
