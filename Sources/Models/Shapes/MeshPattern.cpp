...
namespace acid
{
MeshPattern::MeshPattern(const float& sideLength, const float& squareSize, const uint32_t& vertexCount, const float& textureScale) : m_sideLength(sideLength), m_squareSize(squareSize), m_vertexCount(vertexCount), m_textureScale(textureScale) {}
...
VertexModel MeshPattern::GetVertex(const uint32_t& col, const uint32_t& row)
{
	...
	//=
	// Colour::White;
	return VertexModel(position, uv,
					   normal); // , colour
}
}
