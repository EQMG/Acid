...
class ACID_EXPORT ModelRegister
{
  public:
	...
	void AddMetadata(const std::string& name)
	{
		...
		auto modelCreate = [](const Metadata& metadata) -> std::shared_ptr<Model> { return T::Create(metadata); };
	...
	void AddExtension(const std::string& extension)
	{
		...
		auto modelCreate = [](const std::string& filename) -> std::shared_ptr<Model> { return T::Create(filename); };
