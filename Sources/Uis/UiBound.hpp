...
class ACID_EXPORT UiBound
{
  public:
	...
	/// dimensions.
	/// </param>
	explicit UiBound(const Vector2& position = Vector2(0.0f, 0.0f), const Vector2& reference = UiReference::TopLeft, const BitMask<UiAspect>& aspect = UiAspect::Position | UiAspect::Dimensions, const Vector2& dimensions = Vector2(1.0f, 1.0f));
	...
	const Vector2& GetReference() const
	{
		return m_reference;
	}

	void SetReference(const Vector2& reference)
	{
		m_reference = reference;
	}

	const BitMask<UiAspect>& GetAspect() const
	{
		return m_aspect;
	}

	void SetAspect(const BitMask<UiAspect>& aspect)
	{
		m_aspect = aspect;
	}
