...
/// texture.
/// </summary>
class ACID_EXPORT Cubemap : public NonCopyable, public Descriptor, public Resource
{
  public:
	/// <summary>
	/// Will
	...
	/// texture.
	/// </param>
	static std::shared_ptr<Cubemap> Create(const std::string& filename, const std::string& fileSuffix, const VkFilter& filter = VK_FILTER_LINEAR, const VkSamplerAddressMode& addressMode = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, const bool& anisotropic = true,
	...
	/// called.
	/// </param>
	explicit Cubemap(std::string filename, std::string fileSuffix = ".png", const VkFilter& filter = VK_FILTER_LINEAR, const VkSamplerAddressMode& addressMode = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, const bool& anisotropic = true, const bool& mipmap = true,
	...
	/// texture.
	/// </param>
	Cubemap(const uint32_t& width, const uint32_t& height, std::unique_ptr<uint8_t[]> pixels = nullptr, const VkFormat& format = VK_FORMAT_R8G8B8A8_UNORM, const VkImageLayout& layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
	...
	const std::string& GetFileSuffix()
	{
		return m_fileSuffix;
	};

	const std::vector<std::string>& GetFileSides()
	{
		return m_fileSides;
	};
