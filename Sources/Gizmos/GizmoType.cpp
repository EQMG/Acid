...
GizmoType::GizmoType(std::shared_ptr<Model> model, const float& lineThickness, const Colour& colour) : m_model(std::move(model)), m_lineThickness(lineThickness), m_colour(colour), m_maxInstances(0), m_instances(0), m_instanceBuffer(sizeof(GizmoTypeData) * MAX_INSTANCES) {}
...
void GizmoType::Update(const std::vector<std::unique_ptr<Gizmo>>& gizmos)
{
	...
	// INSTANCE_STEPS
	//*
	// static_cast<uint32_t>(std::ceil(static_cast<float>(gizmos.size())
	///
