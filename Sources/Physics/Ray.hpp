...
class ACID_EXPORT Ray
{
  public:
	...
	const bool& IsUseMouse() const
	{
		return m_useMouse;
	}

	void SetUseMouse(const bool& useMouse)
	{
		m_useMouse = useMouse;
	}

	const Vector2& GetScreenStart() const
	{
		return m_screenStart;
	}

	void SetScreenStart(const Vector2& screenStart)
	{
	...
	const Vector3& GetCurrentRay() const
	{
		return m_currentRay;
	}
