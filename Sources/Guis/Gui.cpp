...
Gui::Gui(UiObject* parent, const UiBound& rectangle, std::shared_ptr<Texture> texture, const Colour& colourOffset)
	: UiObject(parent, rectangle), m_model(ModelRectangle::Create(0.0f, 1.0f)), m_texture(std::move(texture)), m_numberOfRows(1), m_selectedRow(0), m_colourDriver(std::make_unique<DriverConstant<Colour>>(colourOffset))
{
}
