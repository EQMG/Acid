...
class FBuffer : public NonCopyable, public streambuf
{
  public:
	explicit FBuffer(PHYSFS_File* file, const std::size_t& bufferSize = 2048) : m_bufferSize(bufferSize), m_file(file)
	{
...
IFStream::IFStream(const std::string& filename) : BaseFStream(OpenWithMode(filename.c_str(), FileMode::Read)), std::istream(new FBuffer(file)) {}
...
OFStream::OFStream(const std::string& filename, const FileMode& writeMode) : BaseFStream(OpenWithMode(filename.c_str(), writeMode)), std::ostream(new FBuffer(file)) {}
...
FStream::FStream(const std::string& filename, const FileMode& openMode) : BaseFStream(OpenWithMode(filename.c_str(), openMode)), std::iostream(new FBuffer(file)) {}
