...
ParticleType::ParticleType(std::shared_ptr<Texture> texture, const uint32_t& numberOfRows, const Colour& colourOffset, const float& lifeLength, const float& stageCycles, const float& scale)
	: m_texture(std::move(texture)), m_model(ModelRectangle::Create(-0.5f, 0.5f)), m_numberOfRows(numberOfRows), m_colourOffset(colourOffset), m_lifeLength(lifeLength), m_stageCycles(stageCycles), m_scale(scale), m_maxInstances(0), m_instances(0),
...
void ParticleType::Update(const std::vector<Particle>& particles)
{
	...
	// INSTANCE_STEPS
	//*
	// static_cast<uint32_t>(std::ceil(static_cast<float>(particles.size())
	///
	// static_cast<float>(INSTANCE_STEPS)));
	//	m_maxInstances
	//=
	// std::max(m_maxInstances,
	// instances);
