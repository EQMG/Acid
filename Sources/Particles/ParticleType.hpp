...
class ACID_EXPORT ParticleType : public Resource
{
  public:
	...
	/// particle.
	/// </param>
	explicit ParticleType(std::shared_ptr<Texture> texture, const uint32_t& numberOfRows = 1, const Colour& colourOffset = Colour::Black, const float& lifeLength = 10.0f, const float& stageCycles = 1.0f, const float& scale = 1.0f);
	...
	void SetColourOffset(const Colour& colourOffset)
	{
	...
	const float& GetStageCycles() const
	{
		return m_stageCycles;
	}

	void SetStageCycles(const float& stageCycles)
	{
